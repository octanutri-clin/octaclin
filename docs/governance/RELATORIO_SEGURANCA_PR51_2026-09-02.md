...
## 5. Por que relata e nao derruba o boot

Decisao explicita do proprietario neste ciclo, alinhada a licao de 2026-08-22
em `docs/agents/LESSONS_LEARNED.md`: "Health novo medido fora do ambiente real"
-- um check opcional degradou a saude de producao porque a regra foi validada
contra configuracao presumida, e nao contra o ambiente real.

Este ambiente nao tem credencial de Render, Neon, Redis ou Backblaze. Declarar
falha fechada agora significaria apostar que a configuracao produtiva ja atende
as seis regras. Se ela nao atender, o proximo deploy nao sobe -- e o modo de
falha de um gate de seguranca nao pode ser uma indisponibilidade surpresa.

A sequencia acordada e: medir -> coletar evidencia de producao -> converter para
falha fechada em PR proprio e curto. Enquanto isso, uma violacao aparece como
`error` no log do deploy e como `violado` no endpoint de operacoes.

Isso e uma limitacao declarada, nao um gate silenciosamente pulado.

## 6. Validacoes

| Gate | Resultado |
| --- | --- |
| `pnpm --dir octaclin-backend test` | PASS - 170 suites, 1382 testes; 3 suites skipped (testcontainers, sem Docker local) |
| `menor-privilegio-providers.spec.ts` | PASS - 25 testes |
| `servico-menor-privilegio-providers.spec.ts` | PASS - 8 testes |
...
consulta de `pg_roles` negada, `DataSource` nao inicializado, ausencia de
vazamento de host/credencial/role no log e no relatorio, e a garantia de que o
bootstrap nao derruba o processo quando o veredicto e `violado`.

## 7. O que este PR **nao** prova

Nenhuma evidencia de provider foi coletada, porque nenhuma credencial externa
existe neste ambiente e o gate minimo do PR 51 exige "nenhuma mudanca externa
sem aceite humano". Continuam pendentes, para o proprietario:

| Item | Onde |
| --- | --- |
| privilegio real da role de runtime em producao | secao 6.1 da norma |
| Neon: staging e producao como projetos distintos | secao 6.2 |
...
- A verificacao mede o processo que a executa. Um `worker` mal configurado com
  outra `DATABASE_URL` so aparece no log daquele processo; o relatorio do `web`
  nao o cobre.
...
- Enquanto a falha fechada nao entrar, uma violacao depende de alguem ler o log
  do deploy ou chamar o endpoint.

## 9. Rollback

Reverter o commit. As verificacoes sao aditivas: nao alteram conexao, consulta
de negocio, schema ou contrato HTTP existente. O unico acrescimo de superficie e
`GET /operacoes/providers`, ja restrito a `SuperAdmin`.
