...
assert.match(workflow, /MINIO_VERSION: RELEASE\.2025-10-15T17-29-55Z/);
assert.match(workflow, /actions\/setup-go@924ae3a1cded613372ab5595356fb5720e22ba16 # v6/);
assert.match(workflow, /go install github\.com\/minio\/minio@"\$MINIO_VERSION"/);
assert.match(workflow, /MINIO_ROOT_USER="\$ARMAZENAMENTO_S3_ACCESS_KEY_ID"/);
assert.match(workflow, /MINIO_ROOT_PASSWORD="\$ARMAZENAMENTO_S3_SECRET_ACCESS_KEY"/);
assert.match(workflow, /CreateBucketCommand/);
assert.doesNotMatch(workflow, /(?:^|\s)minio\/minio(?::|\s)/m, 'imagem removida do Docker Hub nao pode voltar');
assert.doesNotMatch(workflow, /(?:^|\s)minio\/mc(?::|\s)/m, 'cliente removido do Docker Hub nao pode voltar');
