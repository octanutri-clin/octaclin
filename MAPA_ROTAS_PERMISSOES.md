...
| `/api/agenda/tipos-atendimento` | `/agenda/tipos-atendimento` | GET exige `agenda.consultas.ler`; POST exige `agenda.consultas.criar`. PB-18 (Fase 276): catalogo de tipos de atendimento por tenant (nome + duracao), fonte reutilizavel da duracao ao rotacionar o link publico de agendamento |
| `/api/agenda/tipos-atendimento/[tipoId]` | `/agenda/tipos-atendimento/:tipoId` | DELETE exige `agenda.consultas.criar`; arquiva (`ativo = false`), nao apaga; tipo de outro tenant responde 404, nao 403 |
| `/api/agenda/expediente` | `/agenda/expediente` | GET exige `agenda.consultas.ler`; PUT exige `agenda.consultas.criar`. PB-18 (Fase 276): jornada semanal por profissional; `Professional` sempre le/escreve a propria (resolvida pelo JWT, `profissionalId` do corpo/query ignorado); `SuperAdmin`/`Collaborator` informam `profissionalId` explicito. PUT substitui todas as faixas do profissional numa transacao |
...
| `/api/evolucoes/modelos` | `/evolucoes/modelos` | GET exige `pacientes.ler`; POST exige `pacientes.gerenciar`. Modelo de evolucao clinica nao pertence a paciente (e do profissional ou da clinica), mesma decisao do PB-13/PB-14: a rota fica fora de `/pacientes/[id]`. Sem endpoint de "aplicar" - o cliente le o modelo e o formulario pre-preenche localmente |
| `/api/evolucoes/modelos/[modeloId]` | `/evolucoes/modelos/:modeloId` | GET exige `pacientes.ler`; DELETE exige `pacientes.gerenciar` e arquiva. Modelo `pessoal` so aparece para o profissional dono, filtro aplicado na consulta; modelo pessoal de outro profissional responde 404, nao 403, para nao confirmar que existe |
| `/api/biblioteca-condutas` | `/biblioteca-condutas` | GET exige `pacientes.ler`; POST exige `pacientes.gerenciar`. Biblioteca sempre do tenant inteiro (sem separacao pessoal/clinica); `nome`/`conteudo` cifrados como a conduta terapeutica real, por isso sem busca textual server-side, so filtro por `tipo` (reaproveita o enum `TipoCondutaTerapeutica`) |
| `/api/biblioteca-condutas/[itemId]` | `/biblioteca-condutas/:itemId` | GET exige `pacientes.ler`; DELETE exige `pacientes.gerenciar` e arquiva |
