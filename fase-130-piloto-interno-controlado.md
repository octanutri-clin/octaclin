...
pnpm --dir octaclin-backend test -- servico-portal-cliente.spec.ts servico-usuarios-cliente.spec.ts permissoes.spec.ts servico-pacientes.spec.ts servico-agenda.spec.ts servico-gamificacao.spec.ts --runInBand
...
- O mesmo padrao de escopo do BUG-004 (filtrar por profissional responsavel, nao so por tenant) ainda nao foi auditado nos modulos de comunicacoes, materiais e questionarios. Fica como lacuna conhecida para a proxima rodada do piloto ou uma fase dedicada de hardening de autorizacao.
- Os formularios de criar paciente/circulo/desafio no frontend ainda exibem o seletor de profissional para o papel Professional, mesmo o backend agora ignorando o valor enviado e forcando o proprio profissional. E so uma questao de clareza de UX, nao de seguranca; fica pendente esconder o campo quando a sessao for Professional.
