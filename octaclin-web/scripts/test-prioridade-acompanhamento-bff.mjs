...
const pastaTemporaria = mkdtempSync(join(tmpdir(), 'octaclin-prioridade-acompanhamento-bff-'));
...
writeFileSync(join(pastaTemporaria, 'tsconfig.json'), JSON.stringify({
  ...
  files: [
    'scripts/prioridade-acompanhamento-bff.spec.ts',
    'app/api/pacientes/[id]/prioridade-acompanhamento/route.ts',
...
executar(process.execPath, ['--test', join(pastaTemporaria, 'scripts', 'prioridade-acompanhamento-bff.spec.js')], pastaTemporaria);
rmSync(pastaTemporaria, { recursive: true, force: true });
