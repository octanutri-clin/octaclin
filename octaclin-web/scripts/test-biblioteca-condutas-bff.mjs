...
const temporaria = mkdtempSync(join(tmpdir(), 'octaclin-biblioteca-condutas-bff-'));
const arquivos = [
  'scripts/biblioteca-condutas-bff.spec.ts',
  'app/api/biblioteca-condutas/_proxy.ts',
  'app/api/biblioteca-condutas/route.ts',
  'app/api/biblioteca-condutas/[itemId]/route.ts',
...
executar(process.execPath, ['--test', join(temporaria, 'scripts', 'biblioteca-condutas-bff.spec.js')], temporaria);
rmSync(temporaria, { recursive: true, force: true });
