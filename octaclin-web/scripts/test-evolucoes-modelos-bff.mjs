...
const temporaria = mkdtempSync(join(tmpdir(), 'octaclin-evolucoes-modelos-bff-'));
const arquivos = [
  'scripts/evolucoes-modelos-bff.spec.ts',
  'app/api/evolucoes/modelos/_proxy.ts',
  'app/api/evolucoes/modelos/route.ts',
  'app/api/evolucoes/modelos/[modeloId]/route.ts',
...
executar(process.execPath, ['--test', join(temporaria, 'scripts', 'evolucoes-modelos-bff.spec.js')], temporaria);
rmSync(temporaria, { recursive: true, force: true });
