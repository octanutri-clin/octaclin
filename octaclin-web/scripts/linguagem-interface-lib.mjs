...
const propriedadesVisiveis = new Set([
  ...
  'placeholder', 'rotulo', 'subtitulo', 'texto', 'title', 'titulo'
]);
...
function ehTextoVisivel(no) {
  ...
  if (ts.isJsxExpression(pai) && (ts.isJsxElement(pai.parent) || ts.isJsxFragment(pai.parent))) return true;
  if (ts.isPropertyAssignment(pai) && propriedadesVisiveis.has(nomeNo(pai.name))) return true;
  if (ts.isCallExpression(pai) && ehArgumentoVisivel(no, pai)) return true;
  return false;
}
