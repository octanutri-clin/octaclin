...
const temporaria = mkdtempSync(join(tmpdir(), 'octaclin-exames-bff-'));
const arquivos = [
...
class NextResponse extends Response {
  static json(data, init = {}) { const headers = new Headers(init.headers ?? {}); if (!headers.has('Content-Type')) headers.set('Content-Type', 'application/json'); return new NextResponse(JSON.stringify(data), { ...init, headers }); }
}
...
executar(process.execPath, ['--test', join(temporaria, 'scripts', 'exames-laboratoriais-bff.spec.js')], temporaria);
rmSync(temporaria, { recursive: true, force: true });
