...
export function AreaRollout({ controlador }: { controlador: PainelOperacoesControlador }) {
  ...
  async function salvar() {
    if (!tenantId) {
      setErro('Selecione a clinica que recebera o rollout.');
      return;
    }
    setSalvando(true);
    setErro(null);
    ...
    try {
      ...
      setSucesso('Funcionalidades atualizadas para a clinica selecionada e registradas na auditoria.');
      await carregar();
    } catch (erroAtual) {
      ...
      {rollout ? (
        <>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <Cartao><CartaoCabecalho><h3 className="text-base font-semibold">Filas e integracoes</h3></CartaoCabecalho><div className="divide-y divide-linha">{rollout.filas.map((fila) => <div key={fila.nome} className="flex items-center justify-between gap-3 px-4 py-3 text-sm"><div><strong>{fila.nome.replace('_', ' ')}</strong><p className="mt-1 text-xs text-texto-suave">Aguardando {fila.esperando} | Ativas {fila.ativas} | Atrasadas {fila.atrasadas}</p></div><span className={statusClasse(fila.status)}>{fila.falharam ? `${fila.falharam} falhas` : fila.status}</span></div>)}</div></Cartao>
            <Cartao><CartaoCabecalho><h3 className="text-base font-semibold">Traces sanitizados</h3></CartaoCabecalho><div className="divide-y divide-linha">{rollout.telemetria.tracesRecentes.length ? rollout.telemetria.tracesRecentes.slice(0, 8).map((trace) => <div key={`${trace.requestId}-${trace.horario}`} className="px-4 py-3"><div className="flex items-center justify-between gap-3 text-sm"><strong>{trace.metodo} {trace.rota}</strong><span className={statusClasse(trace.resultado === 'erro_servidor' ? 'critico' : 'ok')}>{trace.statusCode}</span></div><p className="mt-1 text-xs text-texto-suave">{trace.duracaoMs} ms | {trace.requestId}</p></div>) : <EstadoVazio titulo="Sem traces nesta instancia" descricao="As amostras aparecem apos o backend receber requisicoes." />}</div></Cartao>
          </div>

          <Cartao><CartaoCabecalho><div><h3 className="text-base font-semibold">Liberacao controlada</h3><p className="mt-1 text-xs text-texto-suave">IA e Mobile permanecem bloqueados por padrao. Selecione uma clinica para alterar o acesso.</p></div></CartaoCabecalho><CartaoConteudo className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto_auto_auto] md:items-end"><div><Rotulo htmlFor="rollout-tenant">Clinica</Rotulo><Selecao id="rollout-tenant" value={tenantId} onChange={(evento) => setTenantId(evento.target.value)}><option value="">Selecione</option>{tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.nome} ({tenant.slug})</option>)}</Selecao></div><label className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" className="h-4 w-4" checked={iaClinica} onChange={(evento) => setIaClinica(evento.target.checked)} />IA clinica</label><label className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" className="h-4 w-4" checked={mobileSync} onChange={(evento) => setMobileSync(evento.target.checked)} />Mobile</label><Botao type="button" variante="primario" carregando={salvando} onClick={() => void salvar()}><Save size={16} />Aplicar</Botao></CartaoConteudo></Cartao>
        </>
