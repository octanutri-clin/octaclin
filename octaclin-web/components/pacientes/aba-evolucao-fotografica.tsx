...
export function AbaEvolucaoFotografica({ pacienteId, podeGerenciar }: AbaEvolucaoFotograficaProps) {
  ...
  async function registrar(evento: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setEvidencia('');
      setSucesso('Consentimento fotografico registrado. A captura permanece indisponível até a próxima entrega segura.');
      await carregar();
  ...
  async function enviarFoto(evento: FormEvent<HTMLFormElement>) {
    ...
    if (!arquivo) { setErro('Selecione uma imagem para a evolução fotográfica.'); return; }
    ...
    try {
      ...
      setSucesso('Imagem confirmada e vinculada a série fotográfica com consentimento ativo.');
      await carregar();
  ...
  async function confirmarExclusao() {
    ...
    try {
      ...
      setParaExcluir(null);
      setSucesso('Série fotográfica e imagens enviadas foram excluidas permanentemente.');
      await carregar();
