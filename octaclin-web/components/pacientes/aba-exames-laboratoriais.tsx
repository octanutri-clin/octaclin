...
export function AbaExamesLaboratoriais({ pacienteId, podeGerenciar }: AbaExamesLaboratoriaisProps) {
  ...
  async function salvar(evento: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSucesso('Coleta laboratorial registrada no prontuario.');
      await carregar();
    } catch (erroAtual) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 rounded-md border border-linha bg-white p-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-texto-suave">Registre coletas e acompanhe os marcadores sem classificacao automatica.</p>
          </div>
        </div>
      ...
      {podeGerenciar ? (
        <form onSubmit={salvar} className="grid gap-4 rounded-md border border-linha bg-white p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-texto-suave">Os resultados ficam restritos ao prontuario e nao recebem interpretacao pelo sistema.</p>
          </div>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="grid gap-1 md:col-span-2"><Rotulo htmlFor="coleta-laboratorio">Laboratorio</Rotulo><Campo id="coleta-laboratorio" value={formulario.laboratorio} maxLength={180} onChange={(evento) => setFormulario((atual) => ({ ...atual, laboratorio: evento.target.value }))} /></div>
          </div>
          ...
          <div className="grid gap-3 border-y border-linha py-4">
            ...
            {formulario.marcadores.map((marcador, indice) => (
              <div key={indice} className="grid gap-3 rounded-md border border-linha p-3 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_120px_minmax(0,1fr)_minmax(0,1fr)_44px] xl:items-end">
                ...
                <div className="grid gap-1"><Rotulo htmlFor={`marcador-referencia-${indice}`}>Referencia</Rotulo><Campo id={`marcador-referencia-${indice}`} value={marcador.referencia ?? ''} maxLength={160} onChange={(evento) => atualizarMarcador(indice, 'referencia', evento.target.value)} /></div>
                <div className="grid gap-1"><Rotulo htmlFor={`marcador-metodo-${indice}`}>Metodo</Rotulo><Campo id={`marcador-metodo-${indice}`} value={marcador.metodo ?? ''} maxLength={120} onChange={(evento) => atualizarMarcador(indice, 'metodo', evento.target.value)} /></div>
          ...
          <div className="grid gap-1"><Rotulo htmlFor="coleta-observacoes">Observacoes</Rotulo><AreaTexto id="coleta-observacoes" value={formulario.observacoes} maxLength={4000} onChange={(evento) => setFormulario((atual) => ({ ...atual, observacoes: evento.target.value }))} /></div>
