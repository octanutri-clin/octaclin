...
import { BibliotecaCondutas } from './biblioteca-condutas';
...
/** PB-24 (Fase 275): consultaId so vale na criacao; atualizacao de rascunho nao aceita o campo. */
interface FormularioConduta { tipo: TipoCondutaTerapeuticaApi; titulo: string; conteudo: string; validadeInicio: string; validadeFim: string; consultaId: string; }
const formularioInicial: FormularioConduta = { tipo: 'orientacao', titulo: '', conteudo: '', validadeInicio: '', validadeFim: '', consultaId: '' };
...
export function AbaCondutasTerapeuticas({ pacienteId, podeGerenciar }: { pacienteId: string; podeGerenciar: boolean }) {
  ...
  function preencherEdicao(conduta: CondutaTerapeuticaApi) {
    ...
    if (!rascunho) return;
    setEditando(conduta);
    setFormulario({ tipo: conduta.tipo, titulo: rascunho.titulo, conteudo: rascunho.conteudo, validadeInicio: rascunho.validadeInicio ?? '', validadeFim: rascunho.validadeFim ?? '', consultaId: '' });
  }
  ...
  async function salvar(evento: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      else await criarCondutaTerapeutica(pacienteId, { ...entrada, tipo: formulario.tipo, consultaId: formulario.consultaId || undefined });
  ...
  return <section className="grid gap-4">
    ...
    <ModalConfirmacao aberto={Boolean(paraArquivar)} titulo="Arquivar conduta terapêutica" mensagem="A conduta deixara de aparecer como ativa, mas seu histórico versionado será preservado." rotuloConfirmar="Arquivar conduta" confirmando={salvando} aoCancelar={() => setParaArquivar(null)} aoConfirmar={() => { if (paraArquivar) void executar('arquivar', paraArquivar); }} />
  </section>;
}
