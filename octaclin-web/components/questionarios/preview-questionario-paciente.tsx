            ...
            <div key={campo.id} className="grid gap-3 rounded-lg border border-linha bg-[#fbfcfd] p-4">
              ...
              {campo.tipoEntrada === 'radio' || campo.tipoEntrada === 'checkbox' ? (
                <div className="grid gap-2">
                  {campo.opcoes.map((opcao) => (
                    <label key={opcao.valor} className="flex min-h-10 items-center gap-3 rounded-md border border-linha bg-white px-3 py-2 text-sm">
              ...
              {campo.tipoEntrada === 'number' ? (
                <div className="grid grid-cols-[1fr_auto] items-center gap-2">
                  ...
                  />
                  {campo.atributos.unidade ? <span className="text-sm font-medium text-[#596273]">{String(campo.atributos.unidade)}</span> : null}
                </div>
              ) : null}
              ...
              {campo.tipoEntrada === 'file' ? (
                <div className="flex min-h-24 items-center justify-center gap-2 rounded-md border border-dashed border-linha bg-white text-sm text-[#596273]">
                  <ImagePlus size={18} />
                  <span>{String(campo.atributos.accept)} · ate {String(campo.atributos.maxArquivos)} arquivo(s)</span>
                </div>
              ) : null}
