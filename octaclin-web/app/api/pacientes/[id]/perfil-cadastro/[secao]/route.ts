...
export async function GET(_request: Request, props: Params) {
  ...
  if (!secoesLeitura.has(secao)) return NextResponse.json({ mensagem: 'Secao invalida.' }, { status: 404 });
  try {
...
export async function PATCH(request: Request, props: Params) {
  const { id, secao } = await props.params;
  if (!secoesEdicao.has(secao)) return NextResponse.json({ mensagem: 'Secao invalida.' }, { status: 404 });
  try {
