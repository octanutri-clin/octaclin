...
- Onda 4 concluida em `main`: PB-24, PB-18, PB-19, PB-17 e PB-10 foram
  integrados pelos PRs `#305`, `#307`, `#309`, `#313` e `#314`. O merge de
...
- Reconciliacao de 2026-09-19: os Incrementos 266.1 e 266.2 foram integrados
...
- Reconciliacao de 2026-09-20: Fase 268 (PB-05, alerta de check-in com
  ...
  default de produto 50 -- o mesmo corte que a formula de prioridade de
  acompanhamento da Fase 265 ja usa para o fator `adesao_declarada_baixa`,
  para manter os dois sinais de "adesao baixa" do produto consistentes
  entre si. Dispara por check-in individual (nao por rodada): quando a
  adesao declarada em `registrarCheckinRapido` (portal do paciente) fica
  estritamente abaixo do limiar de uma regra ativa do profissional
  responsavel, reutilizando `dispararParaRegra` (nucleo por-regra extraido
  na 267.3) porque a elegibilidade depende do limiar proprio de cada regra
  -- o mesmo motivo que ja levou a essa extracao em `checkin.atrasado`. A
  origem da chave de idempotencia e o proprio registro de check-in, entao
  retry da mesma requisicao nunca duplica; o contexto duravel carrega
  somente o tipo do evento, nunca a adesao declarada, humor ou qualquer
  outro conteudo do check-in. A Web ganhou a opcao "Check-in com adesao
...
- SQ-1C foi integrada pelo PR `#213`, merge `ee9cfed`. O CycloneDX do head
  confirmou que `msgpack`, `setuptools` e o pip global nao existem na imagem IA
  final, enquanto FastAPI, Uvicorn, Pydantic e as dependencias travadas da
  aplicacao permanecem. A recaptura sanitizada pos-merge registrou 216 Code
...
- O gate local `pnpm test:inventario-security-quality` valida o
...
- Durante o preflight da SQ-1C, um comando Git recusado por `safe.directory`
...
- A mesma revisao identificou que o job `AI FastAPI` instalava e auditava o lock,
  mas nao executava a suite declarada na matriz. A SQ-1C conectou o `unittest` ao
  job depois da instalacao por hash e adicionou contrato estatico contra regressao.
...
- Fase 255 concluida e integrada pelo PR `#104`, merge `51a1085`: o prontuario
  ...
  de escrita acompanham as permissoes backend. A busca pontual do responsavel
...
- Fase 254 concluida e integrada. O Incremento 1 entrou pelo PR `#93`, com a
  ...
  A fase seguinte foi a Fase 255.
- Fase 253 concluida e aceita em producao: a agenda interna ganhou trava
  ...
  47/47 migrations. PR `#91`, merge `ea6ed12`, CI `32507735771`, deploys e
  monitor `32508654126` passaram; a proxima fase e a 254.
...
- Fase 229 concluida e aceita em producao: BFF fail-closed, protecao de
  ...
  `31724869285`, ficou `Live` e aprovou os smokes HTTP publicos. O Mobile Expo
  52 permanece fora da oferta e bloqueado pela Fase 241 devido a dependencias
