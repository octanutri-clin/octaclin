...
describe('ServicoMateriais', () => {
  it('deve criar material reutilizavel e enviar ao paciente no contexto do tenant', async () => {
    ...
    const envio = await servico.enviarMaterialParaPaciente('tenant-1', 'paciente-1', 'usuario-profissional-1', {
      materialId: material.id,
      observacao: 'Ler antes do retorno.'
    });
    const envios = await servico.listarMateriaisPaciente('tenant-1', 'paciente-1');
  ...
  it('deve rejeitar envio de material inexistente', async () => {
    ...
    await expect(
      servico.enviarMaterialParaPaciente('tenant-1', 'paciente-1', 'usuario-profissional-1', {
