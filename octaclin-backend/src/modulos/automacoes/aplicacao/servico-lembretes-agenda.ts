...
export class ServicoLembretesAgenda {
  ...
  async processarLembretesConsulta(tenantId: string, agora = new Date()): Promise<ResultadoProcessamentoLembretes> {
    ...
    for (const consulta of consultas) {
      ...
      const email = await this.enviarLembrete(tenantId, 'email', consulta, canais, templates);
      const whatsapp = await this.enviarLembrete(tenantId, 'whatsapp', consulta, canais, templates);
  ...
  private async enviarLembrete(
    ...
    const destino = tipo === 'email' ? this.textoPayload(consulta, 'emailContato') : this.textoPayload(consulta, 'whatsappContato');
